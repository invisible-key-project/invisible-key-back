...
def extract_watermark(image_path):
    def block_extract_watermark(block):
        C_f = block[0, 1]
        C_r = block[1, 0]
    ...
    for pos in block_positions:
        ...
        if lh_block.shape == (block_size, block_size):
            ...
            extracted_key.append(block_extract_watermark(dct_lh_block))
            index += 1

    extracted_key = np.array(extracted_key, dtype=np.uint8)

    # 원래의 IDW와 비교하여 워터마크 추출
    IDW_extracted = []
    index = 0
    ...
    extracted_watermark_image = extracted_watermark_image.astype(np.uint8)
